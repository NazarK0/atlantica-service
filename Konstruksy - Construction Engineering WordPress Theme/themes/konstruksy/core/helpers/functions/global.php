...
// ----------------------------------------------------------------------------------------
function konstruksy_return( $s ) {
   
	return wp_kses_post($s);
}

//css unit check
function konstruksy_style_unit( $data ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_option( $key, $default_value = '', $method = 'customizer' ) {
	if ( defined( 'FW' ) ) {
		switch ( $method ) {
			case 'theme-settings':
...
// ----------------------------------------------------------------------------------------
function konstruksy_meta_option( $postid, $key, $default_value = '' ) {
	if ( defined( 'FW' ) ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_resize( $url, $width = false, $height = false, $crop = false ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_src( $key, $default_value = '', $input_as_attachment = false ) { // for src
	if ( $input_as_attachment == true ) {
		$attachment = $key;
	} else {
		$attachment = konstruksy_option( $key );
	}
...
// ----------------------------------------------------------------------------------------
function konstruksy_org_id( $id, $name = true ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_rgb2hex( $hex ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_google_fonts_url($font_families	 = []) {
	$fonts_url		 = '';
	/*
    Translators: If there are characters in your language that are not supported
    by chosen font(s), translate this to 'off'. Do not translate into your own language.
    */
	if ( $font_families && 'off' !== _x( 'on', 'Google font: on or off', 'konstruksy' ) ) { 
		$query_args = array(
...
// ----------------------------------------------------------------------------------------
function konstruksy_get_mega_item_child_slug( $location, $option_id ) {
	$mega_item	 = '';
...
// ----------------------------------------------------------------------------------------
function konstruksy_youtube_cover( $e ) {
	$src = null;
	//get the url
	if ( $e != '' ){
		$url = $e;
...
// ----------------------------------------------------------------------------------------
function konstruksy_soundcloud_embed( $url ) {
...
// ----------------------------------------------------------------------------------------
function konstruksy_video_embed($url){
...
if ( !function_exists( 'konstruksy_advanced_font_styles' ) ) :

	/**
	 * Get shortcode advanced Font styles
	 *
	 */
	function konstruksy_advanced_font_styles( $style ) {
...
function konstruksy_ekit_headers($format='html'){
...
function konstruksy_ekit_footers($format='html'){
...
function konstruksy_ekit_headers_activate(){

    $header_settings = konstruksy_option('header_builder_enable');
