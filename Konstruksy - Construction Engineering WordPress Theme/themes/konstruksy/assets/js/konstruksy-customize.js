jQuery(document).ready(function ($) {
  ...
  if ($(".konstruksy_header_builder_select").length > 0) {
    function header_builder(current , type) {
        let id = type === 'change' ? current.val() : current.val(),
            admin_url = admin_url_object.admin_url + id;
          current.parents('.fw-backend-option-input-type-multi').find(".konstruksy_header_builder_edit_link").attr("href", admin_url);
    }
    $(".konstruksy_header_builder_select").each(function () {
      header_builder($(this));
